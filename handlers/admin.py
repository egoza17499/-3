...
async def admin_back(callback: types.CallbackQuery):  # <-- БЕЗ , db: Database!
...
async def admin_list(callback: types.CallbackQuery):  # <-- БЕЗ , db: Database!
...
async def admin_stats(callback: types.CallbackQuery):  # <-- БЕЗ , db: Database!
    users = db.get_all_users()
    total = len(users)
